...
# A non-terminal window (here a Firefox-based browser) gets a clipboard paste,
# not injected Unicode — Chromium/Electron need paste, and Firefox-based
# browsers map Shift+Insert to PRIMARY so Ctrl+V is required (issue #10382).
: >"$TMPDIR/hyprctl"
...
# Picker captures origin window address and passes it to the insert helper.
grep -q 'import Quickshell.Hyprland' "$ROOT/shell/plugins/emojis/Emojis.qml" ||
  fail "emojis picker imports Hyprland for origin window address"
pass "emojis picker imports Hyprland for origin window address"

grep -q 'root.targetWindow' "$ROOT/shell/plugins/emojis/Emojis.qml" ||
  fail "emojis picker records targetWindow for mouse origin restore"
pass "emojis picker records targetWindow for mouse origin restore"

grep -q 'args.push(root.targetWindow)' "$ROOT/shell/plugins/emojis/Emojis.qml" ||
  fail "emojis picker passes origin window address to insert helper"
pass "emojis picker passes origin window address to insert helper"

# Helper documents the Firefox PRIMARY / mouse focus issue.
grep -F '#10382' "$ROOT/bin/omarchy-menu-emoji-insert" >/dev/null ||
  fail "emoji insert documents issue #10382"
pass "emoji insert documents issue #10382"
